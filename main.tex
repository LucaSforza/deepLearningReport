...
The implemented hooks are triggered at:
 \begin{enumerate}
   ...
   \item Stop: Is used to guarantee a final state for the model checking, so Halmos must either succeed or timeout othewise the LLM must continue with the generation of candidates.
 \end{enumerate}
...
This does not find us the real minimum, we are solving a black box problem since we cannot know the real $\phi$, 
